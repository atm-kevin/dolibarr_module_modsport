...
/**
 *	\file       modsport/modsportindex.php
 *	\ingroup    modsport
 *	\brief      Home page of modsport top menu
 */
...
$langs->loadLangs(array("modsport@modsport"));

$action = GETPOST('action', 'alpha');


// Security check
//if (! $user->rights->modsport->myobject->read) accessforbidden();
...
if (! empty($conf->modsport->enabled) )
{
	...
	$sql.= " FROM ".MAIN_DB_PREFIX."the_sport_activities as c";
	$resql = $db->query($sql);
	if ($resql)
	{
		$total = 0;
...
if (! empty($conf->modsport->enabled) && $user->rights->modsport->read)
{
