...
/**
 * 	\defgroup   modsport     Module ModSport
 *  \brief      ModSport module descriptor.
 *
 *  \file       htdocs/modsport/core/modules/modModSport.class.php
 *  \ingroup    modsport
 *  \brief      Description and activation file for module ModSport
 */
 ...
 */
class modModSport extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		// Module label (no space allowed), used if translation string 'ModuleModSportName' not found (ModSport is name of module).
		$this->name = preg_replace('/^mod/i', '', get_class($this));
		// Module description, used if translation string 'ModuleModSportDesc' not found (ModSport is name of module).
		$this->description = "ModSportDescription";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = "ModSport description (Long)";
		...
		// Key used in llx_const table to save module status enabled/disabled (where MODSPORT is value of property name of module in uppercase)
		...
		$this->module_parts = array(
			...
			'css' => array(
				//    '/modsport/css/modsport.css.php',
			),
			// Set this to relative path of js file if module must load a js on all pages
			'js' => array(
				//   '/modsport/js/modsport.js.php',
			),
...
//				       'hookcontext2',
		...
		$this->dirs = array("/modsport/temp");
		// Config pages. Put here list of php page, stored into modsport/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@modsport");
		// Dependencies
		...
		$this->langfiles = array("modsport@modsport");
		$this->phpmin = array(5, 5); // Minimum version of PHP required by module
		$this->need_dolibarr_version = array(11, -3); // Minimum version of Dolibarr required by module
		$this->warnings_activation = array(); // Warning to show when we activate module. array('always'='text') or array('FR'='textfr','ES'='textes'...)
		$this->warnings_activation_ext = array(); // Warning to show when we activate an external module. array('always'='text') or array('FR'='textfr','ES'='textes'...)
		//$this->automatic_activation = array('FR'=>'ModSportWasAutomaticallyActivatedBecauseOfYourCountryChoice');
		//$this->always_enabled = true;								// If true, can't be disabled

		// Constants
		// List of particular constants to add when module is enabled (key, 'chaine', value, desc, visible, 'current' or 'allentities', deleteonunactive)
		// Example: $this->const=array(1 => array('MODSPORT_MYNEWCONST1', 'chaine', 'myvalue', 'This is a constant to add', 1),
		//                             2 => array('MODSPORT_MYNEWCONST2', 'chaine', 'myvalue', 'This is another constant to add', 0, 'current', 1)
		// );
		$this->const = array();
		...
		if (!isset($conf->modsport) || !isset($conf->modsport->enabled)) {
			$conf->modsport = new stdClass();
			$conf->modsport->enabled = 0;
		}

		// Array to add new pages in new tabs
		$this->tabs = array();
		// Example:
		// $this->tabs[] = array('data'=>'objecttype:+tabname1:Title1:mylangfile@modsport:$user->rights->modsport->read:/modsport/mynewtab1.php?id=__ID__');  					// To add a new tab identified by code tabname1
		// $this->tabs[] = array('data'=>'objecttype:+tabname2:SUBSTITUTION_Title2:mylangfile@modsport:$user->rights->othermodule->read:/modsport/mynewtab2.php?id=__ID__',  	// To add another new tab identified by code tabname2. Label will be result of calling all substitution functions on 'Title2' key.
		...
		$this->dictionaries=array(
			...
			'tabcond'=>array($conf->modsport->enabled, $conf->modsport->enabled, $conf->modsport->enabled)
		);
		*/

		// Boxes/Widgets
		// Add here list of php file(s) stored in modsport/core/boxes that contains a class to show a widget.
		$this->boxes = array(
			//  0 => array(
			//      'file' => 'modsportwidget1.php@modsport',
			//      'note' => 'Widget provided by ModSport',
		...
		$this->cronjobs = array(
			...
			//      'unitfrequency' => 3600,
			//      'status' => 0,
			//      'test' => '$conf->modsport->enabled',
			//      'priority' => 50,
			//  ),
		);
		// Example: $this->cronjobs=array(
		//    0=>array('label'=>'My label', 'jobtype'=>'method', 'class'=>'/dir/class/file.class.php', 'objectname'=>'MyClass', 'method'=>'myMethod', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>2, 'unitfrequency'=>3600, 'status'=>0, 'test'=>'$conf->modsport->enabled', 'priority'=>50),
		//    1=>array('label'=>'My label', 'jobtype'=>'command', 'command'=>'', 'parameters'=>'param1, param2', 'comment'=>'Comment', 'frequency'=>1, 'unitfrequency'=>3600*24, 'status'=>0, 'test'=>'$conf->modsport->enabled', 'priority'=>50)
		// );
...
//        );
	...
	public function init($options = '')
	{
		...
		$result = $this->_load_tables('/modsport/sql/');
		if ($result < 0) return -1; // Do not activate module if error 'not allowed' returned when loading module SQL queries (the _load_table run sql with run_sql with the error allowed parameter set to 'default')

		// Create extrafields during init
		//include_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
		//$extrafields = new ExtraFields($this->db);
		//$result1=$extrafields->addExtraField('modsport_myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'modsport@modsport', '$conf->modsport->enabled');
		//$result2=$extrafields->addExtraField('modsport_myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'modsport@modsport', '$conf->modsport->enabled');
		//$result3=$extrafields->addExtraField('modsport_myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'modsport@modsport', '$conf->modsport->enabled');
		//$result4=$extrafields->addExtraField('modsport_myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', 0, 0, '', '', 'modsport@modsport', '$conf->modsport->enabled');
		//$result5=$extrafields->addExtraField('modsport_myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'modsport@modsport', '$conf->modsport->enabled');
